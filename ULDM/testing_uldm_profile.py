...
x=2
integral = integrate.quad(lambda y: (1 - (1 + y**2)**(3./2 - 8))/y, 1, x)[0]
...
def phys2ModelParam(m_log10, lambda_factor, theta_E):
    ...
    kappa_0 = lambda_factor**3 * m * np.sqrt(np.pi) * gamma(slope - 0.5)
    kappa_0 = kappa_0 * clight  /(4 * np.pi * Sigma_c * G_const * hbar * a_fit * gamma(slope) )
    M_sol = lambda_factor * clight**3 * hbar * np.sqrt(np.pi) * gamma(slope - 1.5)
    M_sol = M_sol / (G_const * m * a_fit**3 * 4 * gamma(slope) ) # in kg
    M_log10 = np.log10( M_sol/m_sun)
    return kappa_0, theta_c, slope, M_log10
##############################################################################

print(phys2ModelParam(-25,0.0016,1.44))
...
x=10
hypF = (hyp2f1(1, 1 - 2*slope, 2 - 2*slope, -np.sqrt(x**2+1)) )
print('Hyperfunction lensing ', hypF)
