...
if run_sim == True:
    ...
    fitting_kwargs_list = [#['PSO', {'sigma_scale': 1., 'n_particles': 200, 'n_iterations': 200}],
            ['MCMC', {'n_burn': 2000, 'n_run': 3000, 'walkerRatio': 10, 'sigma_scale': .2,
...
if make_cornerPlot == True:
    ...
    labels_new = [r"$ R_{\rm NFW} $", r"$\alpha_{\rm NFW}$", r"$\Sigma_{\rm star}$", r"$ R_{\rm star}$", r"$ h0 $"]
    ...
    if reprocess_corner == True:
        for i in range(len(samples_mcmc)):
            ...
            mcmc_new_list.append([Rs, alpha_Rs, sigma0, Rs_Hernq, h0])
